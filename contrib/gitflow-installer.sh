...
if [ -z "$REPO_HOME" ] ; then
	REPO_HOME="git://github.com/marlesson/gitflow.git"
fi

EXEC_FILES="git-flow"
SCRIPT_FILES="git-flow-init git-flow-feature git-flow-betafix git-flow-hotfix git-flow-release git-flow-support git-flow-version gitflow-common gitflow-shFlags"
